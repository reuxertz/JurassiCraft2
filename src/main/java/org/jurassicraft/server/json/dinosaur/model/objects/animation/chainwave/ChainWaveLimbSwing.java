...
public class ChainWaveLimbSwing extends AnimationCallable<ChainWaveLimbSwing.Info> {

    public ChainWaveLimbSwing(JsonArray array, JsonDinosaurAnimator animator) {
    ...
    public void performAnimations(AnimatableModel model, DinosaurEntity entity, float limbSwing, float limbSwingAmount, float ticks, float rotationYaw, float rotationPitch, float scale) {
        for (Info info : this.list) {
            model.chainWave(info.getRenderers(model), info.speed * this.animator.getGlobalSpeed(), info.degree * this.animator.getGlobalDegree(), info.rootOffset, limbSwing, limbSwingAmount);
        }
    }
