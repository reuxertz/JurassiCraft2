...
public class WorldFossilGen extends WorldGenerator {
    private final Dinosaur dinosaur;
    private final int numberOfBlocks;
    private final Predicate<IBlockState> predicate;


    public WorldFossilGen(Dinosaur dinosaur, int blockCount) {
                            ...
                            for (int j2 = l; j2 <= k1; ++j2) {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D) {
                                    ...
                                    if (state.getBlock().isReplaceableOreGen(state, worldIn, blockpos, this.predicate)) {
                                        worldIn.setBlockState(blockpos, BlockHandler.FOSSIL.getDefaultState(), 2);
                                        TileEntity tileEntity = worldIn.getTileEntity(blockpos);
                                        if(tileEntity instanceof FossilBlock.FossilBlockEntity) {
                                            ((FossilBlock.FossilBlockEntity)tileEntity).setDinosaur(this.dinosaur);
                                        }
                                    }
                                }
                            }
