        ...
        dataBlocks.forEach((pos, type) -> {
            switch (type) {
                ...
                case "FenceGate":
                    ...
                    break;
                case "StainedClay":
                    IBlockState blockState = this.getBiomeSpecificBlockState(Blocks.LOG.getDefaultState());
                    if(blockState.getPropertyKeys().contains(BlockLog.LOG_AXIS)){
                        world.setBlockState(pos, blockState.withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y));
                    }
                    break;
                case "Bricks":
